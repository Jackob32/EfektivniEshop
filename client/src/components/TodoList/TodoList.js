...
class TodoBox extends Component {
    ...
    handleChange(event) {
        ...
        fetch('/update/' + date + '/' + completed + '/' + todo)
            .then(reponse => {
                console.log('updated');
            });
        this.props.load();
    ...
    render() {
        ...
        return <div style={styles.placeholder}>
            <h4>
                <input value={date} checked={this.state.completed} onChange={this.handleChange} type="checkbox"/>
                {todo}
            </h4>
    ...
    render() {
        ...
        <span className="input-group-btn">
          <button className="btn btn-secondary" type="button" onClick={this.handleSubmit}>Save</button>
                ...
                <input className="form-control" type="text" ref={inputElement => this.inputElement = inputElement}
                       placeholder="Enter Reminder"/>
            </form>
            {todos.length >= 1 && <h3 className="text-center">Remainders</h3>}
            {todos.map(({todo, completed, date}) => <div key={date} style={styles.center}>
                <TodoBox date={date} completed={completed} todo={todo} load={this.loadList}/>
            </div>)}
            ...
            {completedtodos.map(({todo, completed, date}, index) => <div style={styles.center} key={date}>
                <div style={styles.placeholder} className="input-group">
                    ...
                    <hr style={styles.smallMargin}/>
                    <h5>
                        <button type="button" value={date} className="btn btn-secondary" onClick={this.handleButton}>
                            remove
                        </button>
                        Created On: {moment(date).format('MMMM Do YYYY, h:mm:ss a')}</h5>
                </div>
            </div>)}
        </div>
    }
